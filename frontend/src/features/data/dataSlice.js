...
// =======================================================
// THUNK UNTUK CRUD RULES
// =======================================================

// GET SEMUA RULES (READ)
export const getRules = createAsyncThunk("data/getRules", async (_, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token;
        const config = { headers: { Authorization: `Bearer ${token}` } };
        const response = await axios.get("/api/rule", config);
        return response.data;
    } catch (error) {
        return handleAsyncError(error, thunkAPI);
    }
...
// CREATE RULE
export const createRule = createAsyncThunk("data/createRule", async (ruleData, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token;
        const config = { headers: { Authorization: `Bearer ${token}` } };
        const response = await axios.post("/api/rule", ruleData, config);
        return response.data;
    } catch (error) {
        return handleAsyncError(error, thunkAPI);
    }
...
// UPDATE RULE
export const updateRule = createAsyncThunk("data/updateRule", async ({ id, ruleData }, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token;
        const config = { headers: { Authorization: `Bearer ${token}` } };
        const response = await axios.put(`/api/rule/${id}`, ruleData, config);
        return response.data;
    } catch (error) {
        return handleAsyncError(error, thunkAPI);
    }
...
// DELETE RULE
export const deleteRule = createAsyncThunk("data/deleteRule", async (id, thunkAPI) => {
    try {
        const token = thunkAPI.getState().auth.user.token;
        const config = { headers: { Authorization: `Bearer ${token}` } };
        await axios.delete(`/api/rule/${id}`, config);
        return id; // Kembalikan ID untuk dihapus di state
    } catch (error) {
        return handleAsyncError(error, thunkAPI);
    }
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateGejala.fulfilled, (state, action) => {
                ...
                state.gejala = state.gejala.map((g) => (g._id === action.payload._id ? action.payload : g));
            })
            ...
            .addCase(deleteGejala.fulfilled, (state, action) => {
                ...
                state.gejala = state.gejala.filter((gejala) => gejala._id !== action.payload);
            })

            // ... di dalam extraReducers
            ...
            .addCase(getRules.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isSuccess = true;
                state.rules = action.payload; // Simpan ke state.rules
            })
            ...
            .addCase(updateRule.fulfilled, (state, action) => {
                state.isLoading = false;
                state.isSuccess = true;
                state.rules = state.rules.map((r) => (r._id === action.payload._id ? action.payload : r));
            })
