...
export const deleteGejala = asyncHandler(async (req, res) => {
    ...
    const gejala = await Gejala.findById(id);

    if (gejala) {
        // 1. Hapus gejalanya
        await Gejala.findByIdAndDelete(id);

        // 2. OTOMATIS hapus rule yang memakai gejala ini (CASCADE DELETE)
        // Ini mencegah "Rule Hantu" jika gejala dihapus tapi rule masih ada
        await Rule.deleteMany({ gejala: id });

        res.json({ message: "Gejala dan Rule terkait berhasil dihapus" });
    } else {
        res.status(404);
